 ...
 * <ul>
 * <li>publish an OSGi-compatible Eclipselink {@link PersistenceProvider}
 * service in the service registry</li>
 * <li>intercept {@link EntityManagerFactory} creation to ensure that the
 * Eclipselink target server (if not specified otherwise) is OSGi compatible</li>
 * </ul>
 */
public class Activator implements BundleActivator, BundleListener {
  ...
  private final ConcurrentMap<Bundle, ServiceRegistration<?>> registeredProviders =
      new ConcurrentHashMap<Bundle, ServiceRegistration<?>>();
    ...
    if (b.getSymbolicName().equals(ECLIPSELINK_JPA_PROVIDER_BUNDLE_SYMBOLIC_NAME)) {
      ...
      if (!!!registeredProviders.containsKey(b)) {
        ...
        ServiceRegistration<?> reg =
            context.registerService(PersistenceProvider.class.getName(), factory, props);
  ...
  /**
   * Get all the relevant packages that the EclipseLink JPA provider exports or
   * persistence packages it uses itself. These are needed so that the woven
   * proxy (for runtime enhancement) can be used later on :)
   * 
   * Note that differently to OpenJPA the relevant classes are actually in more
   * than just one bundle (org.eclipse.persistence.jpa and
   * org.eclipse.persistence.core at the time of this writing). Hence, we have
   * to take more than just the packages of the JPA provider bundle into account
   * ...
   * 
   * @param jpaBundle
   * @return
